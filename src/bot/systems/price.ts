...
class Price extends System {
    ...
    return [
      {
        response:
          ...
          + ': !price set <cmd> <price> | !price unset <cmd> | !price list | !price toggle <cmd>',
        ...opts,
      },
    ];
  }
  ...
  async set(opts: CommandOptions): Promise<CommandResponse[]> {
    ...
    const price = await getRepository(PriceEntity).save({
      ...
      command: cmd,
      price:   parseInt(argPrice, 10),
    });
    const response = prepare('price.price-was-set', {
      command:    cmd,
      amount:     parseInt(argPrice, 10),
      pointsName: getPointsName(price.price),
    });
    return [{ response, ...opts }];
  }
  ...
  async toggle(opts: CommandOptions): Promise<CommandResponse[]> {
    ...
    await getRepository(PriceEntity).save({
      ...price,
      enabled: !price.enabled,
    });
    const response = prepare(
      price.enabled ? 'price.price-was-enabled' : 'price.price-was-disabled',
      { command: cmd },
    );
    return [{ response, ...opts }];
  }
  ...
  async list(opts: CommandOptions): Promise<CommandResponse[]> {
    ...
    const response
      = prices.length === 0
        ? translate('price.list-is-empty')
        : translate('price.list-is-not-empty').replace(
          /\$list/g,
          _.orderBy(prices, 'command')
            .map(o => {
              return `${o.command} - ${o.priceBits} Bits (or ${o.price} Points)`;
            })
            .join(', '),
        );
    return [{ response, ...opts }];
  ...
  async check(opts: ParserOptions): Promise<boolean> {
    ...
    const helpers = (await new Parser().getCommandsList())
      .filter(o => o.isHelper)
      .map(o => o.command);
    ...
    const price = await getRepository(PriceEntity).findOne({
      command: parsed[1],
      enabled: true,
    });
    if (!price) {
    ...
    if (!haveEnoughPoints) {
      const response = prepare('price.user-have-not-enough-points', {
        amount:     removePts,
        priceBits:  price.priceBits,
        command:    `${price.command}`,
        pointsName: getPointsName(removePts),
      });
